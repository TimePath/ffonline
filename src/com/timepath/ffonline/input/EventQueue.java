 ...
 *
 * Events that occur between update calls are placed into the event queue. When
 * update is called, each event is processed, then the event queue is moved to
 * the publicly accessible events.
 * <p>
 * The motivation for this design is three fold. First, events are handled in a
 * thread-safe way (i.e. the list of events wont be modified while the game
 * thread is using it. Second, the program will act more deterministically.
 * While an event may happen at any time, changes to the queue will only occur
 * during the call to {@link #update()}. Finally, through use of the
 * {@link #addEvents(java.util.Collection)} method, user input can be exactly
 * reproduced. Simply add new events in the frame before they are needed, and
...
public abstract class EventQueue<T> {
    ...
    private LinkedList<T> eventQueue = new LinkedList<>();
     ...
     * event queue. Then events is set to the event queue and a new event queue
     * is created. As events is no longer needed, the user is free to modify it
     * however they chose. If a reference is to events is not saved before the
     * next update, its contents will be lost.
     */
